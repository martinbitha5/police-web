...
/**
 * RÃ©cupÃ¨re TOUTES les lignes de la pÃ©riode, par pages de 1000 (les grandes
 * pÃ©riodes dÃ©passent la limite PostgREST).
 *
 ...
 * journÃ©e, hors de portÃ©e sur une annÃ©e, oÃ¹ la requÃªte atteindrait plusieurs
 * dizaines de kilo-octets et serait refusÃ©e par la passerelle HTTP. Ici la
 * requÃªte garde la mÃªme taille quel que soit le nombre de vols.
 *
 * Le tri sur `id` n'est pas dÃ©coratif : sans ordre dÃ©terministe, deux pages
 * successives peuvent renvoyer la mÃªme ligne ou en sauter une. Les feuilles
...
/** Vols de la pÃ©riode, paginÃ©s eux aussi : une annÃ©e dÃ©passe les 1000 vols. */
...
function bagStage(b: Baggage): { label: string; tone: Tone } {
  if (b.cancelled) return { label: b.in_hold && !b.pulled ? 'AnnulÃ© Â· Ã  retirer' : 'AnnulÃ©', tone: 'negative' };
  if (b.kind === 'rush_forward') {
    if (b.rush_status === 'expected') return { label: 'Rush Â· annoncÃ©, pas arrivÃ©', tone: 'neutral' };
    if (b.rush_status === 'pending') return { label: 'Rush Â· Ã  valider', tone: 'warning' };
    if (b.rush_status === 'denied') return { label: 'Rush Â· refusÃ©', tone: 'negative' };
    if (b.arrived) return { label: 'Rush Â· arrivÃ©', tone: 'positive' };
    if (b.in_hold) return { label: 'Rush Â· chargÃ©', tone: 'positive' };
    return { label: 'Rush Â· autorisÃ©', tone: 'warning' };
  }
  if (b.arrived) return { label: 'ArrivÃ© Ã  destination', tone: 'positive' };
  if (b.rush) return { label: 'RÃ©acheminement', tone: 'warning' };
  if (b.in_hold) return { label: 'ChargÃ© en soute', tone: 'positive' };
  if (b.on_dolly) return { label: 'ContrÃ´lÃ© rayon X', tone: 'info' };
  if (b.is_confirmed) return { label: 'EnregistrÃ©', tone: 'neutral' };
...
export async function GET(request: NextRequest) {
  ...
  const to = sp.get('to') ?? '';
  const label = sp.get('label') ?? 'PÃ©riode';
  ...
  if (!auth.user) return NextResponse.json({ error: 'Non authentifiÃ©' }, { status: 401 });
  ...
  if (profile?.role !== 'admin' && profile?.role !== 'supervisor') {
    return NextResponse.json({ error: 'RÃ©servÃ© aux superviseurs et administrateurs' }, { status: 403 });
  }
  ...
  if (flights.length > 0) {
    ...
    // PaginÃ© sur `id`, donc remis dans l'ordre chronologique pour la feuille.
  ...
  // MÃªmes exclusions que partout : passagers hors dÃ©barquÃ©s, bagages passagers
  // hors annulÃ©s. L'expÃ©dition rush est comptÃ©e Ã  part.
  ...
  // Cible de l'arrivÃ©e : ce qui est rÃ©ellement parti en soute (hors rush et
  // annulÃ©s), expÃ©ditions rush comprises.
  ...
  // â”€â”€ Classeur â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const wb = newWorkbook();
  wb.title = `Rapport ${label} ${periodStr}`;

  // FEUILLE 1 â€” SYNTHÃˆSE
  {
    const COLS = 12;
    const ws = addSheet(wb, 'SynthÃ¨se', 'brand');
    let r = titleBand(
      ws,
      {
        title: `Rapport ${label.toLowerCase()}`,
        subtitle: from === to ? `JournÃ©e du ${from}` : `Du ${from} au ${to}`,
        meta: [
          ['PÃ©riode', periodStr],
          ['AÃ©roport', HUB],
          ['Vols traitÃ©s', String(flights.length)],
          ['Ã‰ditÃ© le', now.toLocaleString('fr-FR')],
        ],
      },
      COLS,
    );
    ...
    r = kpiGrid(
      ...
      r,
      [
        { label: 'Vols traitÃ©s', value: flights.length, sub: periodStr, tone: 'brand' },
        { label: 'Passagers', value: totPax, sub: `${totBoarded} embarquÃ©s`, tone: 'brand' },
        {
          label: 'Bagages confirmÃ©s',
          value: totConfirmed,
          sub: `sur ${totDeclared} dÃ©clarÃ©s`,
          tone: 'positive',
        },
        {
          label: 'Alertes fraude',
          value: totAlerts,
          sub: totAlerts > 0 ? 'sur la pÃ©riode' : 'aucune',
    ...
    r = sectionBar(ws, r, 'ActivitÃ©', COLS);
    r = kvRows(
      ws,
      r,
      [
        { label: 'Vols traitÃ©s', value: flights.length },
        { label: 'Passagers enregistrÃ©s', value: totPax },
        { label: 'Passagers embarquÃ©s', value: totBoarded, tone: totBoarded === totPax && totPax > 0 ? 'positive' : undefined },
        { label: 'Reste Ã  embarquer', value: totPax - totBoarded, tone: totPax - totBoarded > 0 ? 'warning' : undefined },
    ...
    r = kvRows(
      ...
      r,
      [
        { label: 'Bagages dÃ©clarÃ©s', value: totDeclared },
        { label: 'Bagages confirmÃ©s au tapis', value: totConfirmed, tone: 'positive' },
        { label: 'ContrÃ´lÃ©s au rayon X (dolly)', value: totOnDolly, tone: totOnDolly > 0 ? 'info' : undefined },
        { label: 'ChargÃ©s en soute', value: totInHold, tone: totInHold > 0 ? 'positive' : undefined },
        { label: 'ArrivÃ©s Ã  destination', value: totArrived, tone: totArrived > 0 ? 'positive' : undefined },
        {
          label: 'Manquants Ã  lâ€™arrivÃ©e',
          value: totMissing,
          // Tant que la rÃ©ception n'a pas commencÃ©, l'Ã©cart n'a pas de sens.
          tone: totArrived > 0 && totMissing > 0 ? 'negative' : totArrived > 0 ? 'positive' : undefined,
        },
        { label: 'Restants (Ã  rÃ©acheminer)', value: totRush, tone: totRush > 0 ? 'warning' : undefined },
        { label: 'ExpÃ©dition rush (sans passager)', value: totRushFwd, tone: totRushFwd > 0 ? 'info' : undefined },
        { label: 'Ã‰cart (dÃ©clarÃ©s âˆ’ confirmÃ©s)', value: totDeclared - totConfirmed, tone: totDeclared - totConfirmed !== 0 ? 'negative' : 'positive' },
    ...
    r = kvRows(
      ...
      r,
      [
        { label: 'Alertes fraude dÃ©tectÃ©es', value: totAlerts, tone: totAlerts > 0 ? 'negative' : 'positive' },
  ...
  // FEUILLE 2 â€” VOLS
  {
    ...
    table(
      ...
      [
        ...
        { header: 'EmbarquÃ©s', width: 12, align: 'right' },
        { header: 'Bag. conf./dÃ©cl.', width: 16, align: 'center' },
        { header: 'Alertes', width: 10, align: 'center' },
      ],
      rows,
      {
        emptyLabel: 'Aucun vol sur la pÃ©riode',
        totals: [`${flights.length} vol(s)`, '', '', totPax, totBoarded, `${totConfirmed} / ${totDeclared}`, totAlerts],
      },
    );
  }

  // FEUILLE 3 â€” PASSAGERS
  {
    ...
    table(
      ...
      [
        ...
        { header: 'PNR', width: 12 },
        { header: 'SiÃ¨ge', width: 8, align: 'center' },
        { header: 'Classe', width: 8, align: 'center' },
        { header: 'Bag. conf./dÃ©cl.', width: 15, align: 'center' },
        { header: 'EmbarquÃ©', width: 11, align: 'center' },
        { header: 'ScannÃ© le', width: 20, align: 'right' },
      ],
      rows,
      { emptyLabel: 'Aucun passager sur la pÃ©riode' },
    );
  }
  ...
  // FEUILLE 4 â€” BAGAGES
  {
    ...
    const rows: Cell[][] = sorted.map((b) => {
      ...
      const st = bagStage(b);
      const soute = b.soute === 'avant' ? 'Soute avant' : b.soute === 'arriere' ? 'Soute arriÃ¨re' : 'N/A';
      const owner =
        b.kind === 'rush_forward'
          ? pax
            ? `${pax.full_name} (restant connu)`
            : 'ExpÃ©dition rush Â· externe'
          : (pax?.full_name ?? 'N/A');
      return [
        f?.date ?? 'N/A',
        f?.flight_number ?? 'N/A',
    ...
    table(
      ...
      [
        { header: 'Date vol', width: 12 },
        { header: 'Vol', width: 12 },
        { header: 'Ã‰tiquette', width: 16 },
        { header: 'SÃ©rie', width: 12 },
        { header: 'Passager', width: 26 },
        { header: 'PNR', width: 12 },
      ...
      rows,
      {
        emptyLabel: 'Aucun bagage sur la pÃ©riode',
        totals: ['', '', `${baggage.length} bagage(s)`, '', '', '', `${totInHold} en soute`, '', `${totOnDolly}`, ''],
      },
    );
  }

  // FEUILLE 5 â€” ALERTES FRAUDE
  {
    ...
    table(
      ...
      [
        ...
        { header: 'Ã‰tiquette', width: 18 },
      ],
      rows,
      { emptyLabel: 'Aucune alerte sur la pÃ©riode' },
    );
  }

  // FEUILLE 6 â€” GRAPHIQUES (pÃ©riodes de plus d'un jour uniquement)
  // Un export d'une seule journÃ©e n'a pas de tendance Ã  tracer : la feuille et
  // les graphiques natifs ne sont ajoutÃ©s que si la pÃ©riode couvre â‰¥ 2 jours.
  let chartSpecs: ChartSpec[] = [];
  if (from !== to) {
    // Jours de la pÃ©riode, continus (les jours sans vol comptent zÃ©ro).
    const days: string[] = [];
    ...
    // AgrÃ©gats par jour (via la date du vol de rattachement).
    ...
    const hr = titleBand(
      ws,
      { title: 'Graphiques', subtitle: periodStr, meta: [['PÃ©riode', periodStr]] },
      6,
    );
    ...
    table(
      ...
      [
        ...
        { header: 'Passagers', width: 12, align: 'right' },
        { header: 'Bag. dÃ©clarÃ©s', width: 14, align: 'right' },
        { header: 'Bag. confirmÃ©s', width: 15, align: 'right' },
        { header: 'Alertes', width: 10, align: 'right' },
      ],
      rows,
      { emptyLabel: 'Aucune donnÃ©e sur la pÃ©riode' },
    );

    // RÃ©fÃ©rences des plages (lignes 1-based : donnÃ©es sous l'en-tÃªte).
    const r0 = hr + 1;
    ...
    chartSpecs = [
      {
        type: 'line',
        title: 'ActivitÃ© par jour',
        categories: cats,
        anchor: { fromCol: 0, fromRow: top, toCol: 12, toRow: top + 20 },
        series: [
          { name: 'Passagers', color: '163300', ref: col('C'), values: of(paxByDay) },
          { name: 'Bagages confirmÃ©s', color: '65CF21', ref: col('E'), values: of(confirmedByDay) },
          { name: 'Alertes fraude', color: 'CB272F', ref: col('F'), values: of(alertsByDay) },
        ],
      },
      {
        type: 'column',
        title: 'Bagages par jour : dÃ©clarÃ©s vs confirmÃ©s',
        categories: cats,
        anchor: { fromCol: 0, fromRow: top + 22, toCol: 12, toRow: top + 42 },
        series: [
          { name: 'DÃ©clarÃ©s', color: 'A8ABA6', ref: col('D'), values: of(declaredByDay) },
          { name: 'ConfirmÃ©s', color: '163300', ref: col('E'), values: of(confirmedByDay) },
        ],
      },
    ];
  }
